...
def transform(source, data_root, label_root, mean, crop_height, crop_width, is_shuffle, is_mirror, phase, show):
    ...
    while True:
        ...
        for data_source, label_source in l:
            ...
            if show:
                print 'original image.shape is', im_data.shape 
                cv2.imshow('ori', im_data)

            # resize image
            resize_scale = 0.8 + 0.2 * np.random.random() / 101L * np.float(100)
            im_data = cv2.resize(im_data, (np.int(im_data.shape[1]*resize_scale), np.int(im_data.shape[0]*resize_scale)), interpolation=cv2.INTER_LINEAR)
            im_label = cv2.resize(im_label, (np.int(im_label.shape[1]*resize_scale), np.int(im_label.shape[0]*resize_scale)), interpolation=cv2.INTER_NEAREST)
            ...
            if pad_height > 0 or pad_width > 0:
                im_data = cv2.copyMakeBorder(im_data, pad_height/2, pad_height - pad_height/2,  pad_width/2, pad_width - pad_width/2, cv2.BORDER_CONSTANT, value=mean)
                im_label = cv2.copyMakeBorder(im_label, pad_height/2, pad_height - pad_height/2,  pad_width/2, pad_width - pad_width/2, cv2.BORDER_CONSTANT, value=255)
            ...
            if show:
                crop_data = np.uint8(im_data)
                cv2.imshow('after crop', crop_data)
            ...
            if show:
                mirror_data = np.uint8(im_data)
                cv2.imshow('after mirror', mirror_data)
                cv2.waitKey(0)
...
class SegReader:
    ...
    def config(self, cfg):
        ...
        crop_height = cfg.get('crop_height', 480)
        crop_width = cfg.get('crop_width', 480)
